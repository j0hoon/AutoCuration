...
파일의 경로를 입력하고 AUTOCURATION ,GTCURATION ,REGISTRATION의 모드를 선택하면 해당하는 json 파일이 생성됨
...
REGISTRATION = False # Registration 단계에서 사용하는 토글
# 현재 REGISTRATION에 대해서만 CN7 작업 끝남. auto, gt 진행 예정
...
class App(QWidget):
    ...
    def __init__(self):
        ...
        # 초기 값 설정
        self.data_name = ''
        self.autocuration = False
        self.gtcuration = False
    ...
    def initUI(self):
        ...
        # 데이터 이름 입력 라벨, 텍스트 박스
        data_label = QLabel('Data Name:')
        self.data_textbox = QLineEdit(self)
        self.data_textbox.textChanged.connect(self.updateDataName)

        # 버튼 그룹
        self.auto_button = QPushButton('AutoCuration', self)
        self.auto_button.setCheckable(True)
        self.auto_button.clicked[bool].connect(self.updateAuto)
        self.gt_button = QPushButton('GTCuration', self)
        self.gt_button.setCheckable(True)
        self.gt_button.clicked[bool].connect(self.updateGT)
        self.reg_button = QPushButton('Registration', self)
        self.reg_button.setCheckable(True)
        self.reg_button.clicked[bool].connect(self.updateReg)

        # 코드 실행 버튼
        run_button = QPushButton('CODE RUN', self)
        run_button.clicked.connect(self.runCode)

        # 버튼 레이아웃
        hbox = QHBoxLayout()
        hbox.addWidget(self.auto_button)
        hbox.addWidget(self.gt_button)
        hbox.addWidget(self.reg_button)

        # 전체 레이아웃
        vbox = QVBoxLayout()
        vbox.addWidget(data_label)
        vbox.addWidget(self.data_textbox)
        vbox.addLayout(hbox)
    ...
    # 데이터 이름 업데이트
    def updateDataName(self, text):
        self.data_name = text

    # AutoCuration 버튼 업데이트
    def updateAuto(self, pressed):
        self.autocuration = pressed

    # GTCuration 버튼 업데이트
    def updateGT(self, pressed):
        self.gtcuration = pressed

    # Registration 버튼 업데이트
    def updateReg(self, pressed):
        self.registration = pressed
    ...
    def runCode(self):
        ...
        matDir = r'\\192.168.75.251\Shares\FOT_Avante Data_1\Rosbag2Mat\CN7_040623'
        makejson = MakeJson(matDir)
        if self.registration:
            makejson.Raw()
        if self.autocuration:
            makejson.AutoCuration()
        if self.gtcuration:
            makejson.GtCuration()
        # self.hide()
